...
function updateQuizFooterButton() {
  ...
  const show = localStorage.getItem('bruxelles_quizEnabled') === 'false';
  quizResumeBtn.style.display = show ? '' : 'none';
}

function setQuizEnabledRuntime(enabled) {
  quizEnabled = enabled;
  localStorage.setItem('bruxelles_quizEnabled', enabled ? 'true' : 'false');
  if (enabled) localStorage.removeItem('bruxelles_quizStopped');
...
function stopQuizPermanently() {
  ...
  localStorage.setItem('bruxelles_quizStopped', 'true');
