/* Experimental POI explorer for CLQ Bruxelles (no production wiring yet). */
(function () {
  "use strict";

  const DEFAULT_CENTER = { lat: 50.8468, lng: 4.3524 }; // Bruxelles
  const FALLBACK_LANG = "fr";
  const LS_LAST_POS = "bruxelles_lastKnownPosition";
  ...
  async function loadPois() {
    const res = await fetch("data/pois_bruxelles_experiment.json");
    if (!res.ok) throw new Error("Impossible de charger pois_bruxelles_experiment.json");
    const data = await res.json();
  ...
  async function loadMasterListPois() {
    // Sections C–J : points non encore géoconfirmés — voir data/pois_bruxelles_masterlist_v1.txt
    return [];
  }
  ...
  function updateTourSavedSummary() {
    ...
    if (!el) return;
    const idxRaw = localStorage.getItem("bruxelles_currentIndex");
    const scoreRaw = localStorage.getItem("bruxelles_score");
